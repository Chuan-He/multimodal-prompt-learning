            ...
            for name, param in self.module.named_parameters():
                if param.requires_grad:
                    ...
                    if grad_func:
                        ...
                        elif "prompt_learner.compound_prompt_projections" in name or 'prompt_learner.proj' in name:
                            if gradient2 == None:
                                gradient = gradient1 + gradient3
                            else:
                                raise NotImplemented
                        else:
                            if "single_layer" in name:
                                gradient = None
                            else:
                                raise NotImplemented
                        # if gradient2 == None:
...
class VNet(nn.Module):
    ...
    def forward(self, grad_ce, grad_regt, grad_regi, param_name):
        name = param_name
        if "prompt_learner.ctx" in name:
            ...
            raise NotImplemented
        grad_ce_regt = grad_ce + grad_regt
        grad_ce_regt_norm = grad_ce_regt / (torch.linalg.norm(grad_ce_regt))
        grad_regi_norm = grad_regi / (torch.linalg.norm(grad_regi))
        # grad_ce_regt_norm = grad_ce_regt / (grad_ce_regt.norm(dim=-1, keepdim=True))
        # grad_regi_norm = grad_regi / (grad_regi.norm(dim=-1, keepdim=True))
        # angle = torch.dot(grads_ce_regt_norm.flatten(), grads_regi_norm.flatten())
        input_grad = torch.cat([grad_ce_regt_norm, grad_regi_norm],dim=-1).float()
        output_grad = torch.sigmoid(self.inet[l_idx](input_grad)).half()
        grad_new = grad_ce_regt + output_grad * grad_regi
