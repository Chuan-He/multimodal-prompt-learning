...
def load_clip_to_cpu(cfg):
    ...
    try:
        ...
        state_dict = torch.load(model_path, map_location="cpu")
    design_details = {"trainer": 'KDPT_MAPLE_1',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.KDPT_MAPLE_1.N_CTX}
    model = clip.build_model(state_dict or model.state_dict(), design_details)
...
class TextEncoder_MaPLe(nn.Module):
    def __init__(self, cfg, clip_model):
        ...
        self.N = cfg.TRAINER.KDPT_MAPLE_1.N
...
class Adapter(nn.Module):
    def __init__(self, c_in, reduction=4):
        ...
        #     nn.ReLU(inplace=True),
        # )
        self.fc = nn.Linear(c_in, c_in, bias=False)
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.KDPT_MAPLE_1.N_CTX
        ctx_init = cfg.TRAINER.KDPT_MAPLE_1.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        N = cfg.TRAINER.KDPT_MAPLE_1.N   # the number of our text prompts 
        #M = len(clip_template[cfg.DATASET.NAME])
        self.N = N #+ M

        self.compound_prompts_depth = cfg.TRAINER.KDPT_MAPLE_1.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        ...
        print('KDPT_MAPLE_1 design: Multi-modal Prompt Learning')
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.text_encoder = TextEncoder_MaPLe(cfg, clip_model)
        self.logit_scale = clip_model.logit_scale
        self.dtype = clip_model.dtype

        self.vision_prompt_number = self.text_prompt_number = self.N = cfg.TRAINER.KDPT_MAPLE_1.N #+ len(clip_template[cfg.DATASET.NAME])
        self.n_cls = len(classnames)
        self.adapter_image = Adapter(512, 4).to(clip_model.dtype)
        #self.adapter_text = Adapter(512, 4).to(clip_model.dtype)
  
        self.image_adapter_m = 0.2
    ...
    def forward(self, image):
        ...
        # score_image = 0.0
        # image_features_pool = image_features.permute(1, 0, 2)
        # for image_feature in image_features_pool:
        #     image_feature = image_feature / image_feature.norm(dim=-1, keepdim=True)
        #     score = cos(image_feature,image_features_ori)
        #     score_image += 1.0 - torch.mean(score)
        score_total = self.w * score_text #+ score_image

        if self.prompt_learner.training:
            return logits, logits_ori, score_total

        return logits

@TRAINER_REGISTRY.register()
class KDPT_MAPLE_1(TrainerX):
    """
    It is based on PLOT.
    """
    
    def check_cfg(self, cfg):
        assert cfg.TRAINER.KDPT_MAPLE_1.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        #self.w = cfg.TRAINER.KDPT_MAPLE_1.W
