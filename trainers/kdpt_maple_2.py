...
def load_clip_to_cpu(cfg):
    ...
    try:
        ...
        state_dict = torch.load(model_path, map_location="cpu")
    design_details = {"trainer": 'KDPT_MAPLE_2',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.KDPT_MAPLE_2.N_CTX}
    model = clip.build_model(state_dict or model.state_dict(), design_details)
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.KDPT_MAPLE_2.N_CTX
        ctx_init = cfg.TRAINER.KDPT_MAPLE_2.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        N = cfg.TRAINER.KDPT_MAPLE_2.N   # the number of our text prompts 
        #M = len(clip_template[cfg.DATASET.NAME])
        self.N = N #+ M

        self.compound_prompts_depth = cfg.TRAINER.KDPT_MAPLE_2.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
        #self.image_feature_shift = nn.Parameter(torch.zeros(1, ctx_dim, device='cuda', dtype=dtype), requires_grad=True)
        #self.text_feature_shift = nn.Parameter(torch.zeros(1, ctx_dim, device='cuda', dtype=dtype), requires_grad=True)
        ...
        print('KDPT_MAPLE_2 design: Multi-modal Prompt Learning')
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.dtype = clip_model.dtype
        #self.image_feature_shift = self.prompt_learner.image_feature_shift
        #self.text_feature_shift = self.prompt_learner.text_feature_shift
        self.image_adapter = Adapter(512).half()
        self.text_adapter = Adapter(512, 4).half()

        self.vision_prompt_number = self.text_prompt_number = self.N = cfg.TRAINER.KDPT_MAPLE_2.N #+ len(clip_template[cfg.DATASET.NAME])
    ...
    def forward(self, image):
        ...
        #image_features = self.image_encoder(image.type(self.dtype), vis_ctx, deeper_vision_prompts) 
        ...
        #text_features = 0.9 * text_features + 0.1 * self.text_adapter(text_features)
        ...
        #image_features = image_features[:,0,:] #### (vision_prompt_number * batch_size, dim)
        ...
        # image_feats = image_features.contiguous().view(batch_size * self.vision_prompt_number, vis_dim)
        # image_feats = image_feats + self.image_feature_shift
        # image_feats = image_feats / image_feats.norm(dim=-1, keepdim=True)
        # text_feats = text_features.contiguous().view(self.prompt_learner.n_cls * self.text_prompt_number, ctx_dim)
        # text_feats = text_feats + self.text_feature_shift
        # text_feats = text_feats / text_feats.norm(dim=-1, keepdim=True)
        # logits = logit_scale * image_feats @ text_feats.t()
        # text_features_pool = text_features.permute(1, 0, 2)
        # logits = []
        # for image_feats in image_features:
        #     norm_image_feats= image_feats / image_feats.norm(dim=-1, keepdim=True)
        #     logits.append(logit_scale * norm_image_feats @ text_feature_pool.t())
        ...
        if self.prompt_learner.training:
            ...
            sim_text = cos(text_feature_pool, text_features_ori)
            cos_text = 1.0 - torch.mean(sim_text)
            sim_image = cos(image_feature_pool, image_features_ori)
            cos_image = 1.0 - torch.mean(sim_image)
            # cos_image = 0
            # loss_scl_image = 0
            # for image_feature in image_features:
            #     image_feature =  image_feature / image_feature.norm(dim=-1, keepdim=True)
            #     sim_image = cos(image_feature, image_features_ori)
            #     cos_image += 1.0 - torch.mean(sim_image)
            #     loss_scl_image += F.l1_loss(image_feature, image_features_ori.cuda(), reduction='mean')
            # cos_image = cos_image / self.N
            # loss_scl_image = loss_scl_image / self.N
            ...
            #     normalized_text_features = text_feature / text_feature.norm(dim=-1,keepdim=True)
            #     loss_scl_text += F.l1_loss(normalized_text_features, text_features_ori.cuda(),
            #                             reduction='mean')
            #     sim_text = cos(normalized_text_features, text_features_ori)
            #     cos_text += 1.0 - torch.mean(sim_text)
            # loss_scl_text = loss_scl_text / self.N
            # cos_text = cos_text / self.N
            #loss_scl_text = F.l1_loss(text_feats, text_features_ori.cuda(),reduction='mean') * 8.0
            #loss_scl_image = F.l1_loss(image_feats, image_features_ori.cuda(),reduction='mean') * 8.0
            # score_image = 0.0
            # image_features = image_features.permute(1, 0, 2)
            # for image_feature in image_features:
            #     image_feature = image_feature / image_feature.norm(dim=-1, keepdim=True)
            #     score = cos(image_feature, image_features_ori)
            #     score_image += 1.0 - torch.mean(score)
            # score_image = score_image / self.N

            # score_text = 0
            # for text_feature in text_features_pool:
            #     text_feature = text_feature / text_feature.norm(dim=-1, keepdim=True)
            #     score = cos(text_feature,text_features_ori)
            #     score_text += 1.0 - torch.mean(score)
            #score_text = score_text / self.N

            return logits, loss_scl_text, loss_scl_image, cos_text, cos_image

        return logits

@TRAINER_REGISTRY.register()
class KDPT_MAPLE_2(TrainerX):
    """
    It is based on PLOT.
    """
    
    def check_cfg(self, cfg):
        assert cfg.TRAINER.KDPT_MAPLE_2.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        for name, param in self.model.named_parameters():
            ...
            if 'image_adapter' in name or 'text_adapter' in name:
    ...
    def forward_backward(self, batch):
        ...
        logits, loss_scl_text, loss_scl_image, cos_text, cos_image = self.model(image)

        loss_ce = F.cross_entropy(logits, label)
        #loss_ref = F.cross_entropy(logits_ref, label)
        #loss_2 = (loss_ce - loss_ref).pow(2)
        ...
        #     log_target=True
        # ) * (1 * 1) / logits.numel()
        loss = loss_ce + self.w1 * loss_scl_text + self.w2 * loss_scl_image  + self.w * cos_text + self.w3 * cos_image
