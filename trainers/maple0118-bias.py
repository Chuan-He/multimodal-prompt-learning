...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.meta_net = nn.Sequential(OrderedDict([
            ("linear1", nn.Linear(ctx_dim, ctx_dim // 8)),
            ("relu", nn.ReLU(inplace=True)),
            ("linear2", nn.Linear(ctx_dim // 8, ctx_dim))
        ]))
    ...
    def forward_vision(self):
        ...
        return self.ctx, self.compound_prompts_vision
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        text_features = []
        for pts_i, image_features, deep_compound_prompts_i in zip(prompts, image_features, deep_compound_prompts_txt):
            text_feature = self.text_encoder(pts_i, tokenized_prompts, list(deep_compound_prompts_i))
            text_feature = text_feature / text_feature.norm(dim=-1, keepdim=True)
            text_features.append(text_feature)
        text_features = torch.stack(text_features)    # b, n_cls, dim

        batch_size = image_features.shape[0]
        n_cls = self.prompt_learner.n_cls
        
        image_features = image_features.view(batch_size, 1, image_features.shape[-1])    #### (vision_prompt_number, batch_size, dim)
        text_features = text_features.view(batch_size, n_cls, text_features.shape[-1])     #### (text_prompt_number, n_cls, dim)
        ...
        sim = torch.einsum('bvd, bnd->vn', image_features, text_features).contiguous()
        sim = sim.view(batch_size * n_cls, 1, 1)
        wdist = 1.0 - sim
        xx = torch.zeros(batch_size*n_cls, 1, dtype=sim.dtype, device=sim.device).fill_(1. / 1)
        yy = torch.zeros(batch_size*n_cls, 1, dtype=sim.dtype, device=sim.device).fill_(1. / 1)
