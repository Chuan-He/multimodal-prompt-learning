...
template_dict = {   'Caltech101': ["a photo of a","this is a photo","this is picture of","one picture of a"], 
                    ...
                    'DescribableTextures':['a photo of a texture', "this is a photo texture","this is texture picture of","one picture of a texture"],
...
def load_clip_to_cpu(cfg):
    ...
    try:
        ...
        state_dict = torch.load(model_path, map_location="cpu")
    design_details = {"trainer": 'KMVPT',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.KMVPT.N_CTX}
    model = clip.build_model(state_dict or model.state_dict(), design_details)
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.KMVPT.N_CTX
        ctx_init_flag = cfg.TRAINER.KMVPT.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        N = cfg.TRAINER.KMVPT.N   # the number of our text prompts 
        ...
        print('KMVPT design: Multi-modal Prompt Learning')

        single_layer = nn.Linear(ctx_dim, 768)
        single_layer.half()
        #self.compound_prompt_projections = _get_clones(single_layer, self.N)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.N = cfg.TRAINER.KMVPT.N #+ len(clip_template[cfg.DATASET.NAME])
    ...
    def forward(self, image):
        ...
        if self.prompt_learner.training:
            #return logits, score_texts, score_images
            return torch.mean(torch.stack(logits), dim=0), score_texts/self.N, score_images/self.N
        
        return torch.mean(torch.stack(logits), dim=0)


@TRAINER_REGISTRY.register()
class KMVPT(TrainerX):
    """
    It is based on PLOT.
    """
    
    def check_cfg(self, cfg):
        assert cfg.TRAINER.KMVPT.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        #if cfg.DATASET.NAME == 'FGVCAircraft':
        #    self.w = 2.0
        #elif  cfg.DATASET.NAME == 'DescribableTextures':
        #    self.w = 1.2
        #elif cfg.DATASET.NAME == 'OxfordFlowers' or cfg.DATASET.NAME == 'Food101':
        #    self.w = 1.0
        #elif cfg.DATASET.NAME == 'DescribableTextures':
        #    self.w = 0.8
        #else:
    ...
    def forward_backward(self, batch):
        ...
        loss = F.cross_entropy(logits, label) + self.w * scores_text + self.w * scores_image
