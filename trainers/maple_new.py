...
def load_clip_to_cpu(cfg):
    ...
    design_details = {"trainer": 'MaPLe_new',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.MAPLE_new.N_CTX}
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.MAPLE_new.N_CTX
        ctx_init = cfg.TRAINER.MAPLE_new.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        vis_dim = 768
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        self.compound_prompts_depth = cfg.TRAINER.MAPLE_new.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
    ...
    def forward(self):
        ...
        return prompts, self.proj(self.ctx), compound_prompts_vision, self.compound_prompts_text
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        prompts, shared_ctx, compound_prompts_vision, compound_prompts_text = self.prompt_learner()
        image_features = self.image_encoder(image.type(self.dtype), shared_ctx, compound_prompts_vision)

        tokenized_prompts = self.tokenized_prompts
        text_features = self.text_encoder(prompts, tokenized_prompts, compound_prompts_text) 
...
class MaPLe_new(TrainerX):
    def check_cfg(self, cfg):
        assert cfg.TRAINER.MAPLE_new.PREC in ["fp16", "fp32", "amp"]
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.MAPLE_new.PREC == "fp32" or cfg.TRAINER.MAPLE_new.PREC == "amp":
            # CLIP's default precision is fp16
            clip_model.float()

        print("Building custom CLIP")
        self.model = CustomCLIP(cfg, classnames, clip_model)
        self.w = cfg.TRAINER.MAPLE_new.W
        ...
        self.scaler = GradScaler() if cfg.TRAINER.MAPLE_new.PREC == "amp" else None
    ...
    def forward_backward(self, batch):
        ...
        prec = self.cfg.TRAINER.MAPLE_new.PREC
        if prec == "amp":
            with autocast():
