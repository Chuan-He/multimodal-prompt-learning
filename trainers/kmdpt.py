...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.compound_prompts_depth = 9 #cfg.TRAINER.KMDPT.PROMPT_DEPTH  # max=12, but will create 11 such shared prompts
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        text_features_pool = self.text_encoder(prompts, tokenized_prompts)
        image_features_pool = self.image_encoder(image.type(self.dtype), vis_ctx) 
        self.dim = image_features_pool.shape[-1]
        self.batch_size = image_features_pool.shape[-2]
