...
template_dict = {   #'Caltech101': ["a photo of a","this is a photo","this is picture of","one picture of a"],
                    'Caltech101': ["a photo of a","this is a photo of","this is picture of","one picture of a"],
                    'DescribableTextures':['a photo of a texture', "this is a photo texture","this is a picture texture","one picture of a texture"],
                    #'DescribableTextures':['a photo of a texture', "this is texture photo of","this is texture picture of","one picture of a texture"],
                    ...
                    #'EuroSAT':['a centered satellite photo of', 'a centered satellite picture of','this is centered satellite photo of','one centered satellite picture of'], 
...
def load_clip_to_cpu(cfg):
    ...
    try:
        ...
        state_dict = torch.load(model_path, map_location="cpu")
    design_details = {"trainer": 'KDPT_M',
                      "vision_depth": 0,
                      "language_depth": 0, "vision_ctx": 0,
                      "language_ctx": 0,
                      "maple_length": cfg.TRAINER.KDPT_M.N_CTX}
    model = clip.build_model(state_dict or model.state_dict(), design_details)
...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        n_ctx = cfg.TRAINER.KDPT_M.N_CTX
        ctx_init_flag = cfg.TRAINER.KDPT_M.CTX_INIT
        dtype = clip_model.dtype
        ctx_dim = clip_model.ln_final.weight.shape[0]
        clip_imsize = clip_model.visual.input_resolution
        cfg_imsize = cfg.INPUT.SIZE[0]
        N = cfg.TRAINER.KDPT_M.N   # the number of our text prompts 
        ...
        print('KDPT_M design: Multi-modal Prompt Learning')
        self.compound_prompts_text = nn.ParameterList([nn.Parameter(torch.empty(n_ctx, 512))
                                                      for _ in range(8)])
        for single_para in self.compound_prompts_text:
            nn.init.normal_(single_para, std=0.02)
        self.single_layer = nn.Linear(ctx_dim, 768)
        self.compound_prompt_projections = _get_clones(self.single_layer, 8)
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.N = cfg.TRAINER.KDPT_M.N #+ len(clip_template[cfg.DATASET.NAME])
    ...
    def forward(self, image):
        ...
        text_features_pool = self.text_encoder(prompts, tokenized_prompts, deeper_text_prompts)
        image_features_pool = self.image_encoder(image.type(self.dtype), vis_ctx, deeper_vision_prompts) 
        self.dim = image_features_pool.shape[-1]
        #image_features_pool = image_features_pool.contiguous().view(self.N, -1, self.dim)
        #self.batch_size = image_features_pool.shape[1]
        text_features_pool = text_features_pool.contiguous().view(self.N, self.n_cls, self.dim)
...
class KDPT_M(TrainerX):
    """
    It is based on PLOT.
    """
    
    def check_cfg(self, cfg):
        assert cfg.TRAINER.KDPT_M.PREC in ["fp16", "fp32", "amp"]
    ...
    def forward_backward(self, batch):
        ...
        logits, score_text, score_image = self.model(image)

        loss = F.cross_entropy(logits, label) + score_text
