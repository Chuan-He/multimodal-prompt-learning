...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.meta_net = nn.Sequential(OrderedDict([
            ...
            #("gelu", QuickGELU()),
            #("bn", nn.BatchNorm1d(ctx_dim*2)),
            ("relu1", nn.ELU(inplace=True)),
            #("linear2", nn.Linear(ctx_dim*2, ctx_dim*2))
            ("linear2", nn.Linear(ctx_dim*2, ctx_dim*2)),
            #("gelu", QuickGELU()),
            #("bn", nn.BatchNorm1d(ctx_dim*2)),
        ...
        self.meta_net = Amortized(
                input_units=ctx_dim, d_theta=ctx_dim // 2, output_units=ctx_dim
            )
    ...
    def forward(self, im_features):
        ...
        bias = self.sample(bias_mu, bias_logvar).unsqueeze(1)  # (b, 1, ctx_dim)
        ...
        for i in range(self.compound_prompts_depth - 1):
            ctx = compound_prompts_text[i].unsqueeze(0)             # (1, n_ctx, ctx_dim)
            bias = self.sample(bias_mu, bias_logvar).unsqueeze(1)
            ctx_shifted = ctx + bias           # (batch, n_ctx, ctx_dim)
        ...
        return prompts, deep_compound_prompts, bias_mu, bias_logvar
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, label=None):
        ...
        #shared_ctx, deep_compound_prompts_vision = self.prompt_learner.forward_vision()
        ...
        prompts, deep_compound_prompts_txt, mu, logvar  = self.prompt_learner(image_features)
        ...
        if self.prompt_learner.training:
            return F.cross_entropy(logits, label) + 0.001 * self.kl_divergence(mu, logvar)

        return logits  
        '''
        #batch, NumClass, Length, dim = prompts.shape
        #prompts = prompts.view(-1, Length, dim)  # (L * NumClass) x Length x DIM
        text_features = []
        for l in range(self.L):
            prompts, deep_compound_prompts_txt, mu, logvar  = self.prompt_learner(image_features)
            deep_compound_prompts_txt = deep_compound_prompts_txt
            text_features_l = []
            for pts_i, deep_compound_prompts_i in zip(prompts, deep_compound_prompts_txt):
                text_features_i = self.text_encoder(pts_i, tokenized_prompts, list(deep_compound_prompts_i))
                text_features_i = text_features_i / text_features_i.norm(dim=-1, keepdim=True)
                text_features_l.append(text_features_i)
            text_features_l = torch.stack(text_features_l) # batch, n_cls, DIM
            text_features.append(text_features_l)
        text_features = torch.stack(text_features)   # L, batch, n_cls, DIM

        image_features = image_features.unsqueeze(0).expand((self.L, -1, -1))
        text_features = text_features / text_features.norm(dim=-1, keepdim=True)
        #text_features = text_features.view(-1, text_features.shape[-2], text_features.shape[-1])  # L *batch, NumClass, DIM   batch=1

        logits = logit_scale * torch.einsum("LBD,LBCD->LBC", image_features, text_features)
