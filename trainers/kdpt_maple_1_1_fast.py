...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        print(f'Layers: {self.compound_prompts_depth} N: {cfg.TRAINER.KDPT_MAPLE_1_1.N} N_CTX: {n_ctx} LR: {cfg.OPTIM_VNET.LR}')
...
class KDPT_MAPLE_1_1(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        logits, loss_scl_text, loss_scl_image = self.model(image)

        loss_ce = F.cross_entropy(logits, label)
        #loss = loss_ce + self.w_t * loss_scl_text + self.w_i * loss_scl_image
        self.optim.zero_grad()
        self.optim_vnet.zero_grad()
        self.vnet.adapt(self.model, loss_ce, self.w_t * loss_scl_text, self.w_i * loss_scl_image)
        self.optim.step()
        self.optim_vnet.step()
