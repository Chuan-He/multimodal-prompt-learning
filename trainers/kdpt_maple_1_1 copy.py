...
class MultiModalPromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        print(f'Layers: {self.compound_prompts_depth} N: {cfg.TRAINER.KDPT_MAPLE_1_1.N} N_CTX: {n_ctx} batch_size: {cfg.DATALOADER.TRAIN_X.BATCH_SIZE} lamda: {cfg.TRAINER.KDPT_MAPLE_1_1.LAMDA} COS:{cfg.TRAINER.COS}')
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image):
        ...
        image_features = (1. - self.lamda) * image_features + self.lamda * self.image_adapter(image_features)
    ...
    def forward_mix(self, image, label):
        ...
        reg_text = F.l1_loss(text_feature_pool, fixed_embeddings,
                                    reduction='mean')
        reg_image = F.l1_loss(image_feature_pool, zero_shot_features,
                                    reduction='mean')
...
class KDPT_MAPLE_1_1(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        if self.COS:
            loss = loss_ce + 0. * cos_text + 0. * cos_image + 15. * loss_scl_text + 5. * loss_scl_image
        else:
            loss = loss_ce + self.w_t * loss_scl_text + self.w_i * loss_scl_image

        self.model_backward_and_update(loss)

        ce_loss, loss_scl_text, loss_scl_image = self.model.forward_mix(image, label)

        if self.COS:
            loss = ce_loss + 15. * loss_scl_text
        else:
            loss = ce_loss + self.w_t * loss_scl_text
