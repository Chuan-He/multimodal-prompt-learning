...
class ResidualAttentionBlock_MMP(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
        ...
        if not self.first_layer:
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :] # 197, 16, 768
                        # Create/configure learnable tokens of this layer
                        visual_context = compound_prompts_deeper[counter]  # extract the correct index, (number_prompt_text, n_ctx, 512) 4, 2, 768
                        # visual_context = visual_context.unsqueeze(1).expand(-1,
                        #                                                       x.shape[1] // visual_context.shape[0],
                        #                                                       -1, -1).contiguous().view(-1,
                        #                                                                    visual_context.shape[-2],
                        #                                                                    visual_context.shape[
                        #                                                                        -1]).permute(1, 0,
                        #                                                                                     2).half()
                        # 
                        visual_context = visual_context.unsqueeze(0).expand(x.shape[1] // visual_context.shape[0], -1, -1, -1).contiguous().view(-1,
                                                                                                      visual_context.shape[-2],
                                                                                                      visual_context.shape[-1]).permute(1, 0, 2).half()
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        suffix = x[1 + self.compound_prompt_nctx:, :, :]
                        # Create/configure learnable tokens of this layer
                        textual_context = compound_prompts_deeper[counter]   ## (number_prompt_text, n_ctx, 512)
                        textual_context = textual_context.unsqueeze(0).expand(x.shape[1] // textual_context.shape[0], -1, -1, -1).contiguous().view(-1, textual_context.shape[-2], textual_context.shape[-1]).permute(1, 0, 2).half()
                        # textual_context = textual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, replaced by previous
                        # layer learnable tokens
                        x = torch.cat([prefix, textual_context, suffix], dim=0)
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'MaPLe' or current_trainer == 'MaPLe_copy' or current_trainer == "MaPLe_0126":
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif 'KDPT_MAPLE' in current_trainer:
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MMP(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
            # Corresponds to default CoOp or CoCoOp
            # assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp' or current_trainer == 'PBPrompt'
            self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers)])

            # self.resblocks = nn.Sequential(*[ResidualAttentionBlock_Sto(width, heads, attn_mask) for _ in range(layers)])
        
        #self.inter_outs = OrderedDict()


    def forward(self, x: torch.Tensor):
        #x1 = x
        #for i in range(self.layers):
        #    x1 = self.resblocks[i](x1)
        #    self.inter_outs[i] = x1[0]
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, patch=False):
...
class VisionTransformer_MMP(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
    ...
    def forward(self, x: torch.Tensor, visual_context, compound_deeper_prompts):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow:
            visual_ctx = visual_context.half()
            x = torch.cat([x, visual_ctx], dim=1)
            # x = x.unsqueeze(0).expand(visual_context.shape[0],-1,-1,-1)
            # visual_ctx = visual_context.unsqueeze(1).expand(-1,x.shape[1],-1,-1)
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or trainer == "MaPLe_copy" or trainer == "MaPLe_0126":
            ...
            elif "KDPT_MAPLE" in trainer:
                self.visual = VisionTransformer_MMP(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
