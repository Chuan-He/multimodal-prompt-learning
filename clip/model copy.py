...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        elif current_trainer == 'MaPLe' or current_trainer == "MaPLe_new" or "MAPLE" in current_trainer or current_trainer == 'KDPT_M':
...
class VisionTransformer_PLOT_MAPLE(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompt, compound_deeper_prompts):
        b = x.shape[0]
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
...
class VisionTransformer_PLOT(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, design_details):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe" or "MaPLe" in trainer or trainer == "KDPT_MAPLE":
            ...
            elif  trainer == 'KDPT_M' or 'MAPLE' in trainer:
                self.visual = VisionTransformer_PLOT_MAPLE(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
            ...
            elif "KDPT" in trainer or "KVPT" in trainer:
                self.visual = VisionTransformer_PLOT(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
            ...
            elif "KMDPT" in trainer or "KMVPT" in trainer:
                self.visual = VisionTransformer_PLOT(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
